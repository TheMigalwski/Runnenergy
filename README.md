...
O projeto utiliza JavaScript para adicionar interatividade à interface e melhorar a experiência do usuário. O script é executado após o carregamento do DOM, garantindo que todos os elementos estejam disponíveis para manipulação.

Uma das funcionalidades implementadas foi a navegação para a tela de login. Ao clicar no elemento com a classe `.login-link`, o usuário é redirecionado automaticamente para a página `login.html`.

Além disso, foram adicionadas interações visuais nos botões de download das lojas (classe `.store-badge`). Ao passar o mouse sobre esses elementos, ocorre uma animação suave com aumento de tamanho (scale) e mudança de cor, utilizando transições em CSS aplicadas via JavaScript. Quando o mouse sai, o botão retorna ao seu estado original.

Essas funcionalidades tornam a interface mais dinâmica e intuitiva, reforçando a usabilidade do sistema.
